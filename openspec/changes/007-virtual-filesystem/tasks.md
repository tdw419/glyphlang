...
## 2. File I/O opcodes
- [x] 2.1 Add `OpOpen`, `OpClose`, `OpRead`, `OpWrite`, `OpSeek` opcodes to the VM opcode table. Each opcode takes the appropriate operands (path string for open, fd + buffer for read/write).
- [x] 2.2 Implement file descriptor table per VM instance: allocate fds on open, release on close, enforce fd limits (default 256 per process).
